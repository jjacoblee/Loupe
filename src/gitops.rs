...
mod tests {
    ...
    fn the_stack_reads_three_real_versions() {
        ...
        // The bare repository's HEAD follows `init.defaultBranch`, which
        // is `master` on a machine that never set it. `remote set-head -a`
        // then has no branch to resolve. Name it, as the work tree is.
        run_git(&["-C", b.as_str(), "symbolic-ref", "HEAD", "refs/heads/main"]).unwrap();
        git(&["init", "-q", "."]);
