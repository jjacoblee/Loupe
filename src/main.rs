...
USAGE: loupe [--pr | --local | --auto] [--theme <name>] [--light | --dark]
  ...
  --acknowledgements
             print the third-party licenses compiled into this binary
  --help     show this help
...
fn parse_cli<I: Iterator<Item = String>>(args: I) -> Result<CliCmd, String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--acknowledgements" => return Ok(CliCmd::Acknowledgements),
...
/// Print every third-party license that ships inside the binary.
///
/// The Rust crates loupe links are covered by THIRD-PARTY-NOTICES.md, which
/// `cargo about` regenerates into each release archive. The syntax and theme
/// definitions are the case that needs this command: `two-face` compiles them
/// straight into the executable, so the licenses printed here are the only
/// copy that travels with a downloaded binary.
fn print_acknowledgements() {
    println!("loupe {}", env!("CARGO_PKG_VERSION"));
    println!("Copyright 2026 Jacob Lee. Apache License 2.0 — see LICENSE and NOTICE.");
    println!();
    println!("Rust crate licenses: THIRD-PARTY-NOTICES.md, shipped in the release");
    println!("archive alongside this binary.");
    println!();
    println!("The syntax definitions and themes below are embedded by the `two-face`");
    println!("crate, curated by the bat project (https://github.com/sharkdp/bat).");
    println!("Every license here requires acknowledgement. The complete listing,");
    println!("including the licenses that do not, is at:");
    println!("{}", two_face::acknowledgement::url());

    let listing = two_face::acknowledgement::listing();
    for (heading, licenses) in [
        ("SYNTAX DEFINITIONS", listing.for_syntaxes()),
        ("THEMES", listing.for_themes()),
    ] {
        if licenses.is_empty() {
            continue;
        }
        println!();
        println!("{:=^76}", format!(" {heading} ({}) ", licenses.len()));
        for license in licenses {
            println!();
            println!("--- {} ---", license.rel_path.display());
            println!("{}", license.text.trim_end());
        }
    }
}
